...
export default function LeadDetailScreen() {
  ...
  async function handleCall() {
    Linking.openURL(`tel:${lead.telefone.replace(/\D/g, "")}`);
  }

  function handleDelete() {
    Alert.alert("Excluir Lead", `Deseja excluir "${lead.nome}"?`, [
      ...
      >
        <View style={[styles.card, { backgroundColor: c.surface, borderColor: c.border }]}>
          <View style={styles.badges}>
            <View style={[styles.badge, { backgroundColor: originBadge.bg }]}>
              <Text style={[styles.badgeText, { color: originBadge.text }]}>{lead.origem}</Text>
            </View>
            <TouchableOpacity
              style={[styles.badge, { backgroundColor: stageBadge.bg }]}
              onPress={() => setStageModal(true)}
            >
              <Text style={[styles.badgeText, { color: stageBadge.text }]}>{lead.stage}</Text>
        ...
        <View style={styles.actionRow}>
          ...
          <TouchableOpacity
            ...
            onPress={() => router.push(`/task/new?leadId=${lead.id}&leadName=${encodeURIComponent(lead.nome)}`)}
          >
        ...
        <View style={[styles.card, { backgroundColor: c.surface, borderColor: c.border, gap: 12 }]}>
          <DetailRow icon="phone" label="Telefone" value={lead.telefone} c={c} />
          {lead.email && <DetailRow icon="mail" label="E-mail" value={lead.email} c={c} />}
          {lead.cpfCnpj && <DetailRow icon="credit-card" label="CPF/CNPJ" value={lead.cpfCnpj} c={c} />}
          {lead.localizacao && <DetailRow icon="map-pin" label="Localização" value={lead.localizacao} c={c} />}
          <DetailRow icon="calendar" label="Cadastrado em" value={formatDate(lead.createdAt)} c={c} />
          <DetailRow icon="clock" label="Atualizado em" value={formatDateTime(lead.updatedAt)} c={c} />
        ...
        {lead.observacoes && (
          <View style={[styles.card, { backgroundColor: c.surface, borderColor: c.border }]}>
            ...
            <Text style={[styles.obsText, { color: c.text }]}>{lead.observacoes}</Text>
          </View>
        )}
      </ScrollView>
              ...
              return (
                <TouchableOpacity
                  ...
                  style={[
                    ...
                    lead.stage === s && { backgroundColor: c.background },
                  ]}
                ...
                >
                  ...
                  {lead.stage === s && <Feather name="check" size={16} color={c.tint} />}
                </TouchableOpacity>
              );
            })}
          </View>
